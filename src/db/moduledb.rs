...
pub async fn fetch_modules_for_course(
    ...
    let modules =
        sqlx::query_as::<_, ModuleInfo>("SELECT id, course_id, title, description, theory FROM modules WHERE course_id = ?")// Todo: Pagination with LIMIT
            .bind(course_id)
...
pub async fn fetch_module(
    ...
    let module = sqlx::query_as::<_, ModuleInfo>("SELECT id, course_id, title, description, theory FROM modules WHERE course_id = ? AND id = ?")
        .bind(course_id)
        .bind(module_id)
